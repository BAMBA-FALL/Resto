...
router.post('/follow/:userId', isAuthenticated, async (req, res) => {
  try {
    ...
    // Ajoutez l'utilisateur suivi à la liste des personnes suivies par le follower
...
router.get('/following/:userId', isAuthenticated, async (req, res) => {
  try {
    ...
    // Récupérez les personnes suivies par l'utilisateur dans la base de données
    const user = await userModel.findById(userId).populate('following', 'username');

    return res.status(200).json({ following: user.following });
  } catch (error) {
    return res.status(500).json({ message: 'Erreur lors de la récupération des personnes suivies.', error: error.message });
  }
});
